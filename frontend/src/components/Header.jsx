...
const ROOM_ICONS = { general: "💬", random: "🎲", tech: "💻", gaming: "🎮" };

export default function Header({ userName, typers, currentRoom, isDM, dmWith }) {
  const others = typers.filter((t) => t !== userName);

  return (
    <div className="relative flex items-center px-5 py-4 border-b border-white/5 bg-white/[0.02] flex-shrink-0">
      <div className="absolute left-0 top-3 bottom-3 w-px bg-gradient-to-b from-transparent via-cyan-400 to-transparent" />

      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className="w-9 h-9 flex-shrink-0 rounded-lg border border-white/10 bg-white/5 flex items-center justify-center text-lg">
          {isDM ? "👤" : (ROOM_ICONS[currentRoom] || "💬")}
        </div>
        <div className="min-w-0">
          <div className="font-display text-white font-bold text-sm tracking-wide">
            {isDM
              ? <><span className="text-gray-500">@</span>{dmWith}</>
              : <><span className="neon-text">#</span>{currentRoom}</>
            }
          </div>
          {others.length > 0 ? (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-cyan-400">{others.join(", ")} typing...</span>
            </div>
          ) : (
            <div className="flex items-center gap-1.5">
              <div className="online-dot" />
              <span className="text-[10px] text-gray-500 tracking-widest uppercase">
                {isDM ? "Direct Message" : "Live"}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 bg-white/5 flex-shrink-0">
        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-cyan-400 to-violet-500 flex items-center justify-center text-[10px] font-bold text-black">
          {userName[0]?.toUpperCase()}
        </div>
        <span className="text-xs text-gray-400 tracking-wide">{userName}</span>
